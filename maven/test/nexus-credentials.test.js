...
test('It loads credentials for Nexus', async () => {
  ...
  expect(loadSecretIntoEnv).toHaveBeenCalledWith('service-account-key', 'nexus-username', 'NEXUS_USERNAME');
  expect(loadSecretIntoEnv).toHaveBeenCalledWith('service-account-key', 'nexus-password', 'NEXUS_PASSWORD');
});
