...
// src/redoc.js
var require_redoc = __commonJS({
  "src/redoc.js"(exports2, module2) {
    ...
    var installRedoc = async () => exec.exec("sudo", [
      "npm",
      "install",
      "redoc-cli",
      "-global"
    ...
    var buildApiDocs = async (openApiYaml, apiName, systemName, version) => exec.exec("redoc-cli", [
      "bundle",
      openApiYaml,
      "-o",
    ...
    var uploadApiDocs = async (bucket, systemName) => exec.exec("gsutil", [
      "cp",
      "-r",
      `${systemName}`,
      `${bucket}systems/`
    ...
    var updateIndex = async (file, addToIndex, title) => {
      const index = `<html>
...
<table style="width:50%;margin-left:auto;margin-right:auto;">
  ...
  ThisWillBeAutoGenerated
    ...
    var uploadIndex = async (fileToUpload, uploadLocation) => exec.exec("gsutil", [
      "cp",
      fileToUpload,
    ...
    var buildIndex = async (apiList, rootFolder, bucket) => {
      let addToIndex = "";
    ...
    var setMetadata = async (folder) => exec.exec("gsutil", [
      "setmeta",
      "-h",
    ...
    var buildAndUploadIndex = async (folder, preFolder, title, bucket) => {
      await fetchDirList(folder).then((linkList) => buildIndex(linkList, folder, bucket).then((addToIndex) => updateIndex(`${title}index.html`, addToIndex, title).then(uploadIndex(`${title}index.html`, preFolder))));
    };
    var deployDocumentation2 = async (yaml, apiName, version, bucket, systemName) => {
      core2.info(`Uploading api documentation for ${apiName}:${version}`);
    ...
    module2.exports = deployDocumentation2;
  }
});
...
var deployDocumentation = require_redoc();
var action = async () => {
