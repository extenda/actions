...
describe('ConfigMap', () => {
  ...
  test('It creates configMap for working directory', async () => {
    const configMap = await createConfigMap({ namespace: 'test' }, 'test-pod/src', '');
    ...
    expect(exec.exec).toHaveBeenCalledWith(
      'kubectl',
      ['create', 'configmap', 'actions-15b1e98', '-n', 'test', '--from-file=test-pod/src'],
    );
  });

  test('It creates configMap for entrypoint', async () => {
    const configMap = await createConfigMap({ namespace: 'test' }, '', 'test-pod/entrypoint.sh');
    expect(configMap).toEqual({
      name: 'actions-15b1e98',
    ...
    expect(exec.exec).toHaveBeenCalledWith(
      'kubectl',
      ['create', 'configmap', 'actions-15b1e98', '-n', 'test', '--from-file=entrypoint.sh=test-pod/entrypoint.sh'],
    );
  });

  test('It creates complete configMap with directory and entrypoint', async () => {
    const configMap = await createConfigMap({ namespace: 'test' }, 'test-pod/src', 'test-pod/entrypoint.sh');
    expect(configMap).toEqual({
      name: 'actions-15b1e98',
      workingDirectory: true,
      entrypoint: true,
  ...
  test('It deletes config map', async () => {
    ...
    expect(exec.exec).toHaveBeenCalledWith('kubectl', ['delete', 'configmap', 'actions-15b1e98', '-n', 'test']);
  });
});
