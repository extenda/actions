...
describe("Check number of pods running", () => {
  ...
  test("It throws error if namespace does not exist", async () => {
    ...
    expect(exec.exec.mock.calls[0][1]).toEqual(getRunningPodsArgs);
  });
  ...
  test("It fails on unknown error", async () => {
    ...
    expect(exec.exec.mock.calls[1][1]).toEqual(getNonRunningPodsArgs); // Checks that the second time exec was called it was with non-running parameters.
  });
