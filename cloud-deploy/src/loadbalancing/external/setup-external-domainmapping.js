...
const getRecordSetIP = async (zone, projectID, host) =>
  gcloudOutput([
    'dns',
    'record-sets',
    'describe',
  ...
  for (const host of hostsToHandle) {
    ...
    for (const dnsZone of DNSZones) {
      if (`${hostZone}.` === dnsZone.dnsName) {
        ...
        core.info(
          `handling ${host} recordsetIP set to currently: ${recordSetIP}`,
        );
        if (!recordSetIP) {
          // create recordset if doesn't exist
          core.info(`creating ${host} with ip: ${recordSetIP}`);

          await createUpdateRecordSet(
            zone,
            projectID,
            host,
            loadBalancerIP,
          ...
          core.info(
            `removing ${host} from certificate handling due to migrate false`,
          );
