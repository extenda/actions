...
describe('getClusterInfo', () => {
  ...
  test('It can find cluster and zone from tribe', async () => {
    execGcloud
      .mockResolvedValueOnce('tribe-staging-12345')
    ...
    expect(execGcloud.mock.calls[0][0]).toEqual(
      expect.arrayContaining([
        '--quiet',
        'projects',
        'list',
        '--filter=NAME~-staging$ AND PROJECT_ID!=tribe-clan-staging-12345',
        '--format=value(PROJECT_ID)',
      ]),
    );
    expect(execGcloud.mock.calls[1][0]).toEqual(
      expect.arrayContaining([
        '--quiet',
        'container',
        'clusters',
  ...
  test('It throws for non-clan project', async () => {
    ...
    await expect(getClusterInfo('tribe-clan-prod-12345')).rejects.toEqual(
      new Error(
        'Could not find GKE project with suffix -prod, or missing permissions to list it.',
      ),
    );
  });
  ...
  test('It throws for missing cluster', async () => {
    ...
    await expect(getClusterInfo('tribe-clan-staging-12345')).rejects.toEqual(
      new Error('Failed to find a GKE cluster in tribe-staging-12345.'),
    );
  });
  ...
  test('It can find cluster zone from this project and cluster', async () => {
    ...
    const cluster = await getClusterInfo('tribe-clan-prod-12345', 'my-cluster');
    expect(cluster).toEqual({
