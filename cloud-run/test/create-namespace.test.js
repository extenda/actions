...
describe('Create namespace', () => {
  ...
  test('It creates namespace if non exists', async () => {
    exec.exec.mockResolvedValueOnce(0)
      ...
      .mockResolvedValue(0);
    await createNamespace('clan_project', true, clusterInfo, 'testns', mockRegoFile);
  ...
  test('It fails on unknown error', async () => {
    exec.exec.mockResolvedValueOnce(0)
      ...
      .mockResolvedValue(0);
    await expect(createNamespace('clan_project', true, clusterInfo, 'testns', mockRegoFile)).rejects.toEqual(new Error('Could not get namespace information! reason: exit code 1'));
  ...
  test('It disables Istio injection', async () => {
    exec.exec.mockResolvedValue(0);
    await createNamespace('clan_project', false, clusterInfo, 'testns', mockRegoFile);
    expect(exec.exec).toHaveBeenCalledWith(
      'kubectl',
