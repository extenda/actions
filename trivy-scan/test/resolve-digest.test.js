...
import {
  default as resolveDigest,
  getImageWithSha256,
} from '../src/resolve-digest.js';
...
test('It resolves linux/amd64 digest for multi-arch images', async () => {
  ...
  const result = await resolveDigest('eu.gcr.io/extenda/test:2.0.0');
...
test('It returns manifest digest with getImageWithSha256', async () => {
  ...
  const result = await getImageWithSha256('eu.gcr.io/extenda/test:3.0.0');
