...
// src/pkgbuilder.js
var require_pkgbuilder = __commonJS({
  "src/pkgbuilder.js"(exports2, module2) {
    var os = require("os");
    ...
    var getBinaryName = () => os.platform() === "win32" ? "InstallerPackageBuilder.Core.Console.exe" : "InstallerPackageBuilder.Core.Console";
    var packageBuilderCommand = async (builder, args) => {
      const {
        builderType,
        packageName,
        workingDir,
    ...
    var downloadBuildTool = async (args) => {
    ...
    var publishPackageCommand = async (args) => {
      const {
        packageName,
        packageVersion,
        outputDir,
      ...
      core2.info(`PublishUrl: ${fullpublishUrl}`);
      core2.info(`user: ${process.env.NEXUS_USERNAME}, pass ${process.env.NEXUS_PASSWORD}`);
      return fetch(fullpublishUrl, {
        ...
        core2.info(`Package published successfully, server responded with ${response.status} ${response.statusText}`);
        return response;
      }).catch((err) => {
        core2.error(`Failed to publish package, server responded with ${err.status} ${err.statusText}`);
        core2.info(`Failed to publish package, server responded with ${err}`);
      });
    };
    var buildPackage2 = async (args) => {
      const {
        builderType,
        publishPackage,
        packageVersion,
...
var { buildPackage } = require_pkgbuilder();
var { checkEnv, run } = require_src5();
var action = async () => {
  try {
