...
describe('action', () => {
  ...
  it.each([false, true])('calls ccc api with correct definitions dryRun=%s', async (dryRun) => {
    ...
    const response = {
      report: [{ id: 'che.receipt-layout.v1', success: true, performedAction: 'created' }],
      success: true,
    };
    const expectCccToBeCalled = mockCccSyncCall('che.receipt-layout.v1', response, dryRun);
